...
import { getActorAttackBonus } from 'src/foundry/dnd5e-compat';
import { buildDeletion, buildRelativeUuid } from 'src/foundry/core-compat';
import {
  getMovementSpeed,
  getSenseLabel,
  getSenseRange,
...
export const FoundryAdapter = {
   ...
   */
  getTidySettingKey(key: string): string {
    return `${CONSTANTS.SETTINGS_KEY_PREFIX}${key}`;
  },
  ...
  async setTidySetting(
    ...
    await game.settings.set(
      CONSTANTS.SETTINGS_NAMESPACE,
      FoundryAdapter.getTidySettingKey(key as string),
      value
    );
  },
  registerTidySetting(key: string, data: any): void {
    game.settings.register(
      CONSTANTS.SETTINGS_NAMESPACE,
      FoundryAdapter.getTidySettingKey(key),
      data
    );
  },
  ...
  getTemplate(templateName: string) {
    // Vendored under female_edition now, not in a tidy5e-sheet module directory.
  ...
  isEffectFavorited(effect: ActiveEffect5e, actor: Actor5e) {
    if (
      ...
      const relativeUuid = buildRelativeUuid(effect, actor);
  ...
  async toggleFavoriteEffect(effect: ActiveEffect5e) {
    ...
    if (favorited) {
      await actor.system.removeFavorite(buildRelativeUuid(effect, actor));
    } else {
      await actor.system.addFavorite({
        type: 'effect',
        id: buildRelativeUuid(effect, actor),
      });
    }
  },
  isItemFavorited(document: any) {
    ...
    if (actor && 'favorites' in actor.system) {
      const relativeUuid = buildRelativeUuid(document, actor);
  ...
  async toggleFavoriteItem(document: any) {
    ...
    if (favorited) {
      await actor.system.removeFavorite(buildRelativeUuid(document, actor));
    } else {
      await actor.system.addFavorite({
        type: 'item',
        id: buildRelativeUuid(document, actor),
      });
    }
  },
