...
import { normalizeContextMenuEntries } from 'src/foundry/core-compat';
...
export default class FloatingContextMenu extends foundry.applications.ux
  ...
  constructor(
    ...
    super(container, selector, menuItems, {
      ...
      onOpen: (target: any) => {
        onOpen?.(target);
        // Core assigns `ui.context = this` before invoking onOpen, so the menuItems
        // the open handler just wrote to ui.context ARE this.menuItems — normalize
        // them here to cover every onOpen path (dynamic item lists, TidyHooks
        // additions, dnd5e's own advancement menu) in one place.
