...
function resolveRow(element: HTMLElement): {
  ...
  const memberUuid = element.closest<HTMLElement>(
    '[data-member-uuid]'
  )?.dataset.memberUuid;
...
export function configureGroupBastionFacilityContextMenu(
  ...
  ui.context.menuItems = [
    ...
    },
    {
      ...
      group: 'common',
      callback: () =>
        facility.sheet.render(true, { mode: CONSTANTS.SHEET_MODE_EDIT }),
    },
    {
      ...
      group: 'common',
      callback: (_element: any, event?: Event) =>
    ...
    },
    {
      ...
      condition: () =>
        canModify() && !isNil(facility.system.progress?.order, ''),
      group: 'common',
      callback: () =>
        facility.update({
