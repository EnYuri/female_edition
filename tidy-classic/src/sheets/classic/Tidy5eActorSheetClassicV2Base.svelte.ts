  ...
  abstract class Tidy5eActorSheetClassicV2Base extends TidyExtensibleDocumentSheetMixin(
    ...
    _getSenses(systemData: any) {
      ...
      for (let [k, label] of Object.entries(CONFIG.DND5E.senses)) {
        ...
        if (v === 0) continue;
        tags[k] = `${getSenseLabel(label)} ${dnd5e.utils.formatLength(
          v,
          units
        )}`;
      }
    ...
    async _onRender(
      ...
      // Tidy documents this as the Classic render hook. The recovered AppV2 sheets
      // never called it, so Classic-specific extensions silently missed every render.
      if (this.element) {
    ...
    async render(...args: unknown[]) {
      ...
      // `this.element` is undefined when the render did not (or no longer) owns a frame:
      // core's `_onSheetChange` closes the old application and re-renders through the new
      // sheet class, so this call can land on an application whose frame is already gone.
      // Without the guard that threw an uncaught TypeError on every sheet-type switch.
