...
import { markDeleted } from 'src/foundry/core-compat';
...
>() {
  ...
  private async _useDefault() {
    ...
    // The whole map is submitted as one value and `update()` merges objects, so the key
    // has to be marked deleted rather than merely absent.
    markDeleted(sectionConfig as Record<string, any>, this.tabId);
