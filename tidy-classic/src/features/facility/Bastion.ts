...
export async function prepareFacilities(
  ...
  for (const facility of Object.values<any>(actor.itemTypes.facility)) {
    const chosenFacilityContext: ChosenFacilityContext =
      await buildChosenFacilityContext(facility);
  ...
  const facilities: CharacterFacilitiesContext = {
...
export async function buildChosenFacilityContext(
  facility: any
): Promise<ChosenFacilityContext> {
  ...
  if (!isNil(level)) {
    subtitle.push(FoundryAdapter.localize('DND5E.LevelNumber', { level: level }));
  }
 ...
 */
export function getOrderCost(chosen: ChosenFacilityContext): number | null {
