...
import { feSetting, feCaptureWorldSettings } from "./fe-gm-priority.js";
...
export * from "./fe-settings-data.js";

const FE_SETTINGS_SERVICES = {
  MODULE_ID, feIsDx3rdSystemId, feSetting, feCaptureWorldSettings,
  feRegisterModuleFolderFonts, feQueryLocalFonts, feLocalFontsSupported, feListCorePriorityKeys,
};

// Settings window is constructed only at init; importing defaults requires no Foundry globals.
// Optional service injection allows isolated testing; normal init uses direct imports.
function feRegisterSettingsMenu({
  MODULE_ID,
  feIsDx3rdSystemId,
  feSetting,
...
} = FE_SETTINGS_SERVICES) {
  const ALL_DEFAULTS = FE_MENU_DEFAULTS;
  // ── Helpers ──
  ...
  class FemaleEditionSettingsMenu extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options = {}) {
      ...
      const values = {
        ...
        stageNameUserColor: feRead("stageNameUserColor"),
    ...
    static async #applyValues(d, { resetDefaults = false } = {}) {
      ...
      const nextChatFontChoice = String(d[S.CHAT_FONT_CHOICE] ?? ALL_DEFAULTS[S.CHAT_FONT_CHOICE]);
  ...
  return FemaleEditionSettingsMenu;
}

Hooks.once("init", () => feRegisterSettingsMenu());

export { feRegisterSettingsMenu };
