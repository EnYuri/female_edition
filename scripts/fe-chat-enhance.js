...
function feApplyStyleClassesToDocument(doc = document) {
  ...
  // archive/PDF exported while "유저 로컬 폰트" (user local font) was active always rendered
  // in the default system stack ("PDF/HTML로 인쇄하면 기본 고딕으로 나온다" = printing to
  // PDF/HTML comes out in the default gothic). The CSS
...
Hooks.once("init", () => {
  ...
  // (browser) button, and feOpenArchiveInExternalBrowser's mode gate). The registration is
...
Hooks.on("clientSettingChanged", (fullKey, value) => {
  try {
    ...
    if (keyPath.startsWith("core.")) {
      ...
      // this the enforcement would only hold until the player opened 환경 설정 (the
  ...
  try {
    ...
    if (game.user?.isGM && feSetting(S.GM_SPEAK_AS_SELF)) {
      try {
        ...
        // speak-as-self override. Theatre's "없음" (none) choice means only that THEATRE does
        ...
        if (!stageActive) {
          ...
          // color, which reads as "midi 의 테두리 색 설정이 GM 색으로 고정됐다" (midi's
          // border-color setting got stuck on the GM color). These are
