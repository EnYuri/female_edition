...
// reached by typing 0 in the "설정" (settings) dialog, not by collapsing the frame.
...
class ScreenPanelSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  static async #onEditOverlay(event, target) {
     ...
     * "0 = 자동" (0 = auto) placeholder that is the only place that rule is stated in the UI.
     * A
    ...
    await foundry.applications.api.DialogV2.prompt({
      ...
      // without it the fully-expanded bar block pushes the "적용" (apply) footer past the
      // window
      // bottom and .window-content's `overflow: hidden` clips it away entirely.
      classes: ["fe-sp-overlay-edit-dialog"],
      content,
      // The bar's eight fields are dead weight while "값 바 표시" (show value bar) is off —
      // and they were the
